...
import { Lock, ShieldCheck, Eye, EyeOff, ArrowRight, KeyRound } from 'lucide-react';

const FRONTEND_ENV_PWD = (import.meta.env.VITE_ADMIN_PASSWORD || import.meta.env.VITE_ADMIN_PASSCODE || '').trim();
...
export default function AdminAuthGate({ children }) {
  const [isAuthenticated, setIsAuthenticated] = useState(() => {
    return localStorage.getItem('falcon_admin_session') === 'true' || sessionStorage.getItem('falcon_admin_session') === 'true';
  });
  ...
  const handleLogin = async (e) => {
    ...
    if (!cleanInput) {
      triggerShake('Please enter your admin password.');
      return;
    }

    // 1. Direct check if VITE_ADMIN_PASSWORD is set in frontend build
    if (FRONTEND_ENV_PWD && cleanInput === FRONTEND_ENV_PWD) {
      if (rememberMe) {
        localStorage.setItem('falcon_admin_session', 'true');
        localStorage.setItem('falcon_admin_token', cleanInput);
      } else {
    ...
    // 2. Server-side check against FALCON_ADMIN_PASSWORD on backend
    setIsVerifying(true);
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        triggerShake(data.detail || 'Incorrect admin password. Please try again.');
      ...
      triggerShake('Network error verifying password: ' + err.message);
    } finally {
      setIsVerifying(false);
    }
  };
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: '28px' }}>
          ...
          <p style={{
            ...
            Single-administrator console. Enter your admin password to unlock.
          </p>
        </div>
        ...
        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
          ...
          <button
            ...
            onMouseOver={(e) => { if (!isVerifying) e.currentTarget.style.transform = 'translateY(-1px)'; }}
            onMouseOut={(e) => { e.currentTarget.style.transform = 'translateY(0)'; }}
          >
            <span>{isVerifying ? 'Verifying Password...' : 'Unlock Admin Console'}</span>
            <ArrowRight size={16} />
          </button>
        </form>
